...
def health_check():
    return jsonify({"status": "healthy", "message": "Mock server is running (Email Fix)"})
...
def login():
    ...
    # Check if username matches a username in mock_users
    ...
    # Check if username matches an email in mock_users
    for user_key, user_data in mock_users.items():
        if user_data.get('email') == username and password == "password":
            user = user_data.copy()
            return jsonify({
                "message": "Login successful",
                "user": user,
...
if __name__ == '__main__':
    print("🚀 Starting Fixed Mock API Server (Email Support) on http://localhost:8080")
    print("📝 Available test credentials:")
    print("   - Username: testuser, Password: password")
    print("   - Email: aishashaikh@gmail.com, Password: password")
